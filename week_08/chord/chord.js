...
d3.csv("../degrees.csv").then((data) => {
  ...
  let arr1 = d3.map(data, d => d.source),
    arr2 = d3.map(data, d => d.target);  //Take each value and push into new array
  let names = arr1.concat(arr2);

  names = [...new Set(names)]; //spread syntax-this expands everything in array, keeping only unique values
